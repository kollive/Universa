import {
  ...
  const attribApi = {
    ...
    insWPlanTable(userData) {
      debugger;
      //console.log(userData.user);
      //console.log(userData.password);
  
      //new Promise((resolve, reject) => {
      return fetch("http://hvs.selfip.net:4003/insWPlanTable/", {
  ...
  function* insertWPlanTable(userData) {
    try {
      yield put({
      ...
      const resultObj = yield call(attribApi.insWPlanTable, userData.payload);
      //debugger;
      if (resultObj.response && !resultObj.response.ok) {
        ...
        //debugger;
        const fetchTask = yield call(getWPlanTable, {
    ...
    try {
      switch (action.type) {
        ...
        case wplanTypes.INSERT_REQUEST: {
          //yield all([put({ type: "LOGIN_STATUS", message: '' }), put({ type: "ITEMS_IS_LOADING", isLoading: true })])
          debugger;
          const fetchTask = yield fork(insertWPlanTable, action.payload);
          debugger;
          break;
        }
